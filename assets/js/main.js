...
class ChadGPTStore {
    ...
    sanitizeText(text) {
        return text ? text.toString().replace(/[<>&"']/g, (char) => {
            const entities = {
                '<': '&lt;',
                '>': '&gt;',
                '&': '&amp;',
    ...
    performSearch(query) {
        ...
        if (filteredProducts.length === 0) {
            ...
            noResultsDiv.style.cssText = 'grid-column: 1 / -1; text-align: center; padding: 2rem;';
            
            const heading = document.createElement('h3');
            heading.textContent = `No products found for "${query}"`;
            
            const paragraph = document.createElement('p');
            paragraph.textContent = 'Try a different search term or browse all products.';
